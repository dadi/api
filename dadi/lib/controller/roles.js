...
Roles.prototype.deleteResource = function(req, res, next) {
  ...
  return acl.access
    ...
    .then(() => {
      return acl.role.resourceRemove(req.params.role, req.params.resource)
    })
    .then(({results}) => {
      help.sendBackJSON(204, res, next)(null, null)
    })
  ...
  return acl.access
    ...
    .then(access => {
      ...
      if (req.body.extends && !acl.client.isAdmin(req.dadiApiClient)) {
        return acl.access
          .getClientRoles(req.dadiApiClient.clientId)
          .then(roles => {
...
Roles.prototype.put = function(req, res, next) {
  if (typeof req.params.role !== 'string' || typeof req.body !== 'object') {
    return help.sendBackJSON(400, res, next)(null, {
      success: false,
