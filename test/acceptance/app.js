	...
	describe('HTTP2 request', function () {
	    ...
	    it('should start a server', function (done) {
	        app.start(function (err) {
	            ...
	            setTimeout(function () {
	            	var doc_link = 'https://localhost:'+config.get('server.port') + '/serama/config';
	    ...
	    describe('collections api', function () {
	        ...
	        describe('POST', function () {
	            ...
	            it('should create new documents', function (done) {
	                ...
	                needle.post(doc_link, {field1: 'foo!'}, options, function(err, res) {
	                    if (err) return done(err);
	                    should(res.statusCode).be.equal(200);
	                    should.exist(res.body.results);
	                    res.body.results.should.be.Array;
	            ...
	            it('should create new documents when body is urlencoded', function (done) {
	                ...
	                needle.post(doc_link, body, options, function(err, res) {
	                    if (err) return done(err);
	                    should(res.statusCode).be.equal(200);
	                    should.exist(res.body.results);
	            ...
	            it('should update existing documents', function (done) {
	                ...
	                needle.post(doc_link, {field1: 'doc to update'}, options, function(err, res) {
	                    if (err) return done(err);
	                    should(res.statusCode).be.equal(200);
	                    var doc = res.body.results[0];
	                    should.exist(doc);
	                    doc.field1.should.equal('doc to update');
	                    needle.post('https://localhost:'+config.get('server.port') + '/vtest/testdb/test-schema/' + doc._id, {field1: 'updated doc'}, options, function(err, res) {
	                        if (err) return done(err);
	                        should(res.statusCode).be.equal(200);
	                        res.body.results[0]._id.should.equal(doc._id);
	                        res.body.results[0].field1.should.equal('updated doc');
	                        needle.get('https://localhost:'+config.get('server.port') + '/vtest/testdb/test-schema?filter={"_id": "' + doc._id + '"}', options, function(err, res) {
	                            if (err) return done(err);
	                            should(res.headers['content-type']).be.match(/json/);
                      			should(res.statusCode).be.equal(200);
	                            res.body['results'].should.exist;
	        ...
	        describe('GET', function () {
	            ...
	            it('should get documents from correct API version', function (done) {
	                ...
	                help.createDocHttps(bearerToken, function (err, doc) {
		                ...
		                var doc_link = 'https://localhost:'+config.get('server.port') + '/v1/testdb/test-schema/config';
		                var options = url.parse(doc_link);
		                options.key = fs.readFileSync(config.get('server.http2.key_path'));
		                options.ca = fs.readFileSync(config.get('server.http2.crt_path'));
		                options.headers = {
		                ...
		                needle.post(doc_link, schema, options, function(err, res) {
	                        ...
	                        setTimeout(function () {
	                            ...
	                            var testdoc = { newField: "test string" };
	                            help.createDocWithSpecificVersionHttps(bearerToken, 'v1', testdoc, function (err, doc) {
	                                if (err) return done(err);

	                                setTimeout(function () {
	                                	var doc_link = 'https://localhost:'+config.get('server.port') + '/v1/testdb/test-schema';
						                var options = url.parse(doc_link);
						                options.key = fs.readFileSync(config.get('server.http2.key_path'));
						                options.ca = fs.readFileSync(config.get('server.http2.crt_path'));
						                options.headers = {
	            ...
	            it('should allow case insensitive query', function (done) {
	                ...
	                help.createDocWithParamsHttps(bearerToken, doc, function (err) {
	                   ...
	                   if (err) return done(err);
	                    ...
	                    query = encodeURIComponent(JSON.stringify(query));
	                    var doc_link = 'https://localhost:'+config.get('server.port') + '/vtest/testdb/test-schema?cache=false&filter=' + query;
		                var options = url.parse(doc_link);
		                options.key = fs.readFileSync(config.get('server.http2.key_path'));
		                options.ca = fs.readFileSync(config.get('server.http2.crt_path'));
		                options.headers = {
	            ...
	            it('should return specified fields only when supplying `fields` param', function (done) {
	               ...
	               var doc = { field1: "Test", field2: null };
	                ...
	                help.createDocWithParamsHttps(bearerToken, doc, function (err) {
	                    ...
	                    query = encodeURIComponent(JSON.stringify(fields));
	                    var doc_link = 'https://localhost:'+config.get('server.port') + '/vtest/testdb/test-schema?cache=false&fields=' + query;
		                var options = url.parse(doc_link);
		                options.key = fs.readFileSync(config.get('server.http2.key_path'));
		                options.ca = fs.readFileSync(config.get('server.http2.crt_path'));
		                options.headers = {
	            ...
	            it('should find specific document using filter param', function (done) {
	               help.createDocHttps(bearerToken, function (err, doc1) {
	                    if (err) return done(err);
	                   help.createDocHttps(bearerToken, function (err, doc2) {
	                        if (err) return done(err);
	                        ...
	                        query = encodeURIComponent(JSON.stringify(query));
	                        var doc_link = 'https://localhost:'+config.get('server.port') + '/vtest/testdb/test-schema?filter=' + query;
			                var options = url.parse(doc_link);
			                options.key = fs.readFileSync(config.get('server.http2.key_path'));
			                options.ca = fs.readFileSync(config.get('server.http2.crt_path'));
			                options.headers = {
	            ...
	            it('should find specific document using request param', function (done) {
	               help.createDocHttps(bearerToken, function (err, doc1) {
	                   ...
	                   help.createDocHttps(bearerToken, function (err, doc2) {
	                        ...
	                        var docId = doc2._id
	                        var doc_link = 'https://localhost:'+config.get('server.port') + '/vtest/testdb/test-schema/' + doc2._id;
			                var options = url.parse(doc_link);
			                options.key = fs.readFileSync(config.get('server.http2.key_path'));
			                options.ca = fs.readFileSync(config.get('server.http2.crt_path'));
			                options.headers = {
	            ...
	            it('should find documents when using request param and filter', function (done) {
	               help.createDocHttps(bearerToken, function (err, doc1) {
	                   ...
	                   help.createDocHttps(bearerToken, function (err, doc2) {
	                        ...
	                        query = encodeURIComponent(JSON.stringify(query));
	                        var doc_link = 'https://localhost:'+config.get('server.port') + '/vtest/testdb/test-schema/' + doc2._id + '?filter=' + query;
			                var options = url.parse(doc_link);
			                options.key = fs.readFileSync(config.get('server.http2.key_path'));
			                options.ca = fs.readFileSync(config.get('server.http2.crt_path'));
			                options.headers = {
	            ...
	            it('should find one document using a standard query with count=1', function (done) {
	               help.createDocHttps(bearerToken, function (err, doc1) {
	                   ...
	                   help.createDocHttps(bearerToken, function (err, doc2) {
	                        ...
	                        var docId = doc2._id
	                        var doc_link = 'https://localhost:'+config.get('server.port') + '/vtest/testdb/test-schema?count=1&cache=false';
			                var options = url.parse(doc_link);
			                options.key = fs.readFileSync(config.get('server.http2.key_path'));
			                options.ca = fs.readFileSync(config.get('server.http2.crt_path'));
			                options.headers = {
	            ...
	            it('should return single document when querystring param count=1', function (done) {
	                ...
	                ac.on('ready', function () {
	                    ...
	                    var query = {};
	                    query = encodeURIComponent(JSON.stringify(query));
	                    var doc_link = 'https://localhost:'+config.get('server.port') + '/vtest/testdb/test-schema?count=1';
		                var options = url.parse(doc_link);
		                options.key = fs.readFileSync(config.get('server.http2.key_path'));
		                options.ca = fs.readFileSync(config.get('server.http2.crt_path'));
		                options.headers = {
	            ...
	            describe('query string params', function () {
	                before(function (done) {
	                    ...
	                    for (var i = 0; i < 45; ++i) {
	                       help.createDocHttps(bearerToken, function (err) {
	                            if (err) return asyncControl.emit('error', err);
	                            count += 1;
	                ...
	                it('should paginate results', function (done) {
	                    var docCount = 20;
	                    var doc_link = 'https://localhost:'+config.get('server.port') + '/vtest/testdb/test-schema?page=1&count=' + docCount;
		                var options = url.parse(doc_link);
		                options.key = fs.readFileSync(config.get('server.http2.key_path'));
		                options.ca = fs.readFileSync(config.get('server.http2.crt_path'));
		                options.headers = {
	                ...
	                it('should return correct pagination nextPage value', function (done) {
	                    var docCount = 20;
						var doc_link = 'https://localhost:'+config.get('server.port') + '/vtest/testdb/test-schema?page=2&count=' + docCount;
		                var options = url.parse(doc_link);
		                options.key = fs.readFileSync(config.get('server.http2.key_path'));
		                options.ca = fs.readFileSync(config.get('server.http2.crt_path'));
		                options.headers = {
	                ...
	                it('should show later pages', function (done) {
	                	var doc_link = 'https://localhost:'+config.get('server.port') + '/vtest/testdb/test-schema?count=20&page=1';
		                var options = url.parse(doc_link);
		                options.key = fs.readFileSync(config.get('server.http2.key_path'));
		                options.ca = fs.readFileSync(config.get('server.http2.crt_path'));
		                options.headers = {
		                ...
		                needle.get(doc_link, options, function(err, res) {
	                        ...
	                        var eleventhDoc = res.body['results'][10];
	                        needle.get('https://localhost:'+config.get('server.port') + '/vtest/testdb/test-schema?count=10&page=2', options, function(err, res) {
	                            if (err) return done(err);
		                        should(res.headers['content-type']).be.match(/json/);
	              				should(res.statusCode).be.equal(200);
	                            res.body['results'].should.exist;
	                ...
	                it('should allow sorting results', function (done) {
	                	var doc_link = 'https://localhost:'+config.get('server.port') + '/vtest/testdb/test-schema?sort=field1';
		                var options = url.parse(doc_link);
		                options.key = fs.readFileSync(config.get('server.http2.key_path'));
		                options.ca = fs.readFileSync(config.get('server.http2.crt_path'));
		                options.headers = {
	                ...
	                it('should allow specifying descending sort order', function (done) {
	                	var doc_link = 'https://localhost:'+config.get('server.port') + '/vtest/testdb/test-schema?sort=field1&sortOrder=desc';
		                var options = url.parse(doc_link);
		                options.key = fs.readFileSync(config.get('server.http2.key_path'));
		                options.ca = fs.readFileSync(config.get('server.http2.crt_path'));
		                options.headers = {
	                ...
	                it('should allow specifying ascending sort order', function (done) {
	                	var doc_link = 'https://localhost:'+config.get('server.port') + '/vtest/testdb/test-schema?sort=field1&sortOrder=asc';
		                var options = url.parse(doc_link);
		                options.key = fs.readFileSync(config.get('server.http2.key_path'));
		                options.ca = fs.readFileSync(config.get('server.http2.crt_path'));
		                options.headers = {
	                ...
	                it('should return javascript if `callback` is provided', function (done) {
	                    var callbackName = 'testCallback';
	                    var doc_link = 'https://localhost:'+config.get('server.port') + '/vtest/testdb/test-schema?callback=' + callbackName;
		                var options = url.parse(doc_link);
		                options.key = fs.readFileSync(config.get('server.http2.key_path'));
		                options.ca = fs.readFileSync(config.get('server.http2.crt_path'));
		                options.headers = {
		                ...
		                needle.get(doc_link, options, function(err, res) {
	                        if (err) return done(err);
	                        should(res.headers['content-type']).be.equal('text/javascript');
              				should(res.statusCode).be.equal(200);
	                        res.body.slice(0, callbackName.length).should.equal(callbackName);
	                        res.body.slice(-2).should.equal(');');
	                        done();
	                    });
	                });
	            });
	        ...
	        describe('DELETE', function () {
	            ...
	            it('should remove documents', function (done) {
			        ...
			        needle.post(doc_link, {field1: 'doc to remove'}, options, function(err, res) {
	                    if (err) return done(err);
	                    should(res.statusCode).be.equal(200);
	                    var doc = res.body.results[0];
	                    should.exist(doc);
	                    doc.field1.should.equal('doc to remove');
	                    needle.delete('https://localhost:'+config.get('server.port') + '/vtest/testdb/test-schema/' + doc._id, null, options, function(err, res) {
	                    	if (err) return done(err);
	            ...
	            it('should remove a specific document', function (done) {
	               help.createDocHttps(bearerToken, function (err, doc1) {
	                   ...
	                   help.createDocHttps(bearerToken, function (err, doc2) {
	                        ...
	                        var doc_link = 'https://localhost:'+config.get('server.port') + '/vtest/testdb/test-schema/' + doc1._id;
					        var options = url.parse(doc_link);
					        options.key = fs.readFileSync(config.get('server.http2.key_path'));
					        options.ca = fs.readFileSync(config.get('server.http2.crt_path'));
					        options.headers = {
					        ...
					        needle.delete(doc_link, null, options, function(err, res) {
	                            if (err) return done(err);
	                            should(res.statusCode).be.equal(204);
	                            var filter = encodeURIComponent(JSON.stringify({
	                                _id: doc2._id
	                            }));
	                            needle.get('https://localhost:'+config.get('server.port') + '/vtest/testdb/test-schema?filter=' + filter, options, function(err, res) {
	                            	if (err) return done(err);
						            should(res.headers['content-type']).be.match(/json/);
						            should(res.statusCode).be.equal(200);
	            ...
	            it('should return a message if config.feedback is true', function (done) {
			        ...
			        needle.post(doc_link, {field1: 'doc to remove 2'}, options, function(err, res) {
	                    if (err) return done(err);
	                    should(res.statusCode).be.equal(200);
	                    var doc = res.body.results[0];
	                    should.exist(doc);
	        ...
	        describe('Collection stats', function () {
	          ...
	          it('should respond to a stats method', function (done) {
	            help.createDocHttps(bearerToken, function (err, doc) {
	              	...
	              	var doc_link = 'https://localhost:'+config.get('server.port') + '/vtest/testdb/test-schema/stats';
			      	var options = url.parse(doc_link);
			      	options.key = fs.readFileSync(config.get('server.http2.key_path'));
			        options.ca = fs.readFileSync(config.get('server.http2.crt_path'));
			        options.headers = {
	          ...
	          it('should return 404 if not a GET request', function (done) {
	            help.createDocHttps(bearerToken, function (err, doc) {
			        ...
			        needle.post(doc_link, {}, options, function(err, res) {
		                if (err) return done(err);
