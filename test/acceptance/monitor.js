...
describe('File system watching', function () {
    ...
    describe('HTTP2 Request', function () {
        ...
        describe('changing files', function () {
            ...
            it('should update collections component when file changes', function (done) {
                this.timeout(4000);
                var doc_link = 'https://localhost:'+config.get('server.port') + '/vtest/testdb/monitor-test-schema';
                ...
                needle.post(doc_link, {field1: 'string value'}, options, function(err, res) {
                    if (err) return done(err);
                    ...
                    setTimeout(function () {
                        needle.post(doc_link, {field1: 31337}, options, function(err, res) {
                            if (err) return done(err);
                            should(res.headers['content-type']).be.match(/json/);
                            should(res.statusCode).be.equal(200);
                            done();
            ...
            it('should update endpoint component when file changes', function (done) {
                var doc_link = 'https://localhost:'+config.get('server.port') + '/v1/monitor-test-endpoint?cache=false';
                var options = url.parse(doc_link);
                options.key = fs.readFileSync(config.get('server.http2.key_path'));
                options.ca = fs.readFileSync(config.get('server.http2.crt_path'));
                options.headers = {
        ...
        describe('adding new files', function () {
            ...
            it('should add to collections api when file is added', function (done) {
                ...
                // allow time for app to respond
                setTimeout(function () {
                    var doc_link = 'https://localhost:'+config.get('server.port') + '/vtest2/testdb/new-test-schema';
                    var options = url.parse(doc_link);
                    options.key = fs.readFileSync(config.get('server.http2.key_path'));
                    options.ca = fs.readFileSync(config.get('server.http2.crt_path'));
                    options.headers = {
            ...
            it('should add to endpoints api when file is added', function (done) {
                ...
                setTimeout(function () {
                    var doc_link = 'https://localhost:'+config.get('server.port') + '/v1/new-test-endpoint';
                    var options = url.parse(doc_link);
                    options.key = fs.readFileSync(config.get('server.http2.key_path'));
                    options.ca = fs.readFileSync(config.get('server.http2.crt_path'));
                    options.headers = {
