...
describe('Hook', function () {
  ...
  describe('`beforeCreate` hook', function () {
    ...
    it('should not insert documents that fail asynchronous beforeCreate processing', function (done) {
      ...
      const settings = { database: 'testdb', storeRevisions: false, hooks: { beforeCreate: [{ hook: 'slug', options: { from: 'title', to: 'slug' } }] } }
    ...
    it('should modify single documents after create', function (done) {
      ...
      mod.create({fieldName: 'foo', title: 'Article One', slug: ''}, function (err, result) {
        ...
        mod.find({fieldName: 'foo'}, function (err, doc) {
          ...
          const file = path.resolve(path.join(__dirname, 'testAfterCreateHook.log'))
    ...
    it('should modify documents after update', function (done) {
      ...
      mod.create({fieldName: 'foo', title: 'Article One', slug: ''}, function (err, result) {
        ...
        mod.update({ _id: id }, {title: 'Article Two'}, function (err, doc) {
          ...
          const fs = require('fs')
          const path = require('path')
          const file = path.resolve(path.join(__dirname, 'testAfterUpdateHook.log'))
        ...
        mod.delete({ _id: id }, function (err, doc) {
          ...
          setTimeout(function () {
            ...
            mod.find({ _id: id }, function (err, doc) {
              ...
              const fs = require('fs')
              const path = require('path')
              const file = path.resolve(path.join(__dirname, 'testBeforeDeleteHook.log'))
