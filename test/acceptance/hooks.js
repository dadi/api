...
describe('Hooks', function() {
  beforeEach(done => {
    ...
    help.writeTempFile('temp-workspace/hooks/hook1.js', hookOne, callback => {
  ...
  it('testing failure when specifying _layout as a field in a GET request', function(done) {
    const article = {
      ...
      heroImage: ['57a47713a510df48125e052e'],
      heroImageLandscape: [
        {
          width: 778,
          height: 436,
    ...
    startApp(() => {
      sinon
        .stub(hook.Hook.prototype, 'load')
        .returns(require(__dirname + '/temp-workspace/hooks/layout.js'))
    ...
    startApp(() => {
      ...
      client
        ...
        .end(function(err, res) {
          ...
          res.body[0].details.should.eql(
            'hook1 - Missing return statement on beforeCreate hook'
          )
...
slugifyHook +=
  '  // if (type === "beforeUpdate" || type === "beforeCreate") {\n'
slugifyHook += '    var object = Object.assign({}, obj)\n'
slugifyHook +=
  '    var field = data.options ? getFieldValue(data.options.override, object) || getFieldValue(data.options.from, object) : null\n'
